...
rlnode SCHED[5];                         /* The scheduler queue */
...
void sched_queue_add(TCB* tcb)
{
  ...
  rlist_push_back(& SCHED[tcb->priority], & tcb->sched_node);  ///11111111111111
  Mutex_Unlock(& sched_spinlock);

  /* Restart possibly halted cores */
  cpu_core_restart_one();
}
void sched_queue_add_TOZERO(TCB* tcb)
{
  /* Insert at the end of the scheduling list */
  Mutex_Lock(& sched_spinlock);
  rlist_push_back(& SCHED[0], & tcb->sched_node);  ///11111111111111
  Mutex_Unlock(& sched_spinlock);
...
*/
TCB* sched_queue_select(int currPriority)   // ******** ARXIKOPOIHSH SE MHDEN *********************/////////
{
  Mutex_Lock(& sched_spinlock);
  rlnode * sel = rlist_pop_front(& SCHED[currPriority]);
  Mutex_Unlock(& sched_spinlock);
...
void BoostAll(rlnode *SCHED){ /*111111111111111111111111111111111111111111*/
    int i=0;
    for(i=1;i<4;i++){
        rlist_append(SCHED,&SCHED[i]);
        fprintf(stderr,"SPONGEBOB_SQUAREPANTS");
    }
}
...
currPriority=0;
  /* Get next */
    ...
    TCB* next ;
    next = sched_queue_select(currPriority);
while(next==NULL){
	currPriority++;
	next = sched_queue_select(currPriority);
		if(currPriority>3)
...
void gain(int preempt)
{
  ...
  if(preempt) preempt_on;
    ...
    if(current->priority<4 && current->phase==CTX_CLEAN)
      current->priority++;
